...
// List Controller 
.controller("ListSkills", ["$route", "$rootScope", "$scope", "$filter", "$window", "$timeout", "$location", "notificationService", "skillService", 
	function($route, $rootScope, $scope, $filter, $window, $timeout, $location, notificationService, skillService) {
	...
	$scope.delete = function(id)
	{
		...
		{
			$scope.toasts = notificationService.showSuccessNotification('Registro eliminado con exito');
			$timeout(closeAlert, 5000);
			//createToast('Registro eliminado con exito');
		});
		/*
...
.controller("NewSkill", ["$scope", "$window", "skillService", "skillTypeService", function($scope, $window, skillService, skillTypeService)
{
	$scope.skillTypes;
